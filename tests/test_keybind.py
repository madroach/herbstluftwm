...
def test_keys_inactive_affects_release_binds(hlwm, keyboard):
    ...
    hlwm.attr.clients[winid].keys_inactive = '(Release[-+])?x'
...
def test_key_release_of_modifier(hlwm, keyboard):
    hlwm.call(['new_attr', 'string', 'my_test', 'initial'])
    hlwm.call(['keybind', 'Any-Super_L', 'set_attr', 'my_test', 'press'])
    hlwm.call(['keybind', 'Release-Any-Super_L', 'set_attr', 'my_test', 'release'])
    assert hlwm.attr.my_test() == 'initial'
    keyboard.down('alt')
